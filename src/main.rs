...
const DIR: &str = "/home/user/snap/bitcoin-core/common/.bitcoin/regtest/"; // TODO: don't hardcode this
...
fn init_wallet(rpc: &Client) -> (Address, transaction::Transaction, usize) {
    ...
    rpc.create_wallet(&(DIR.to_owned() + WALLET), None, None, None, None);
    rpc.load_wallet(&(DIR.to_owned() + WALLET));
